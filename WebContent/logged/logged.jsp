	...
	<div class="middle" style="min-height:600px;height:auto;">
		<div class="list">
			<ul class="list-ul">
				...
				<a href="loggedAction.jsp?id=1" target="tabframe" style="text-decoration:none;">待填写</a></li>
				<li class="list-li" id="li2" onclick="tab(2)" >
				<a href="loggedAction.jsp?id=2" target="tabframe" style="text-decoration:none;">已填写</a></li>
				<li class="search list-li" id="li4" onclick="tab(3)">
					<select id="searchType" style="border:none;outline:none;background:none;float:left;margin-top:3px;">
						<option value="3" style="border:none;outline:none;background:none;">未填写</option>
						<option value="4" style="border:none;outline:none;background:none;">已填写</option>
					</select>
					<input type="text" id="searchKey" class="search-key" style="width:168px;">
					<img src="../image/search.png" class="search-img" onclick="searchLogged()"></li>
			</ul>
		</div><!--ä¸­é¨å¤´-->
		...
		<div style="overflow: hidden;min-height:600px;height:auto;">
			<iframe src="loggedAction.jsp?id=1" name="tabframe" frameborder=0 scrolling=no HEIGHT=auto WIDTH=100% id="iframe"></iframe>
		</div><!--å·²åæ¡-->
...
<script type="text/javascript">
	...
	function tab(j){
		if(i!=3)document.getElementById('li'+i).style.borderBottom="none";
		if(j!=3) document.getElementById('li'+j).style.borderBottom="3px solid yellow";
		i=j;
	}
	
	function searchLogged(){
		$("#iframe").attr('src',"loggedAction.jsp?id="+$("#searchType").val()+"&key="+$("#searchKey").val());
	}
</script>
